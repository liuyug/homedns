...
def ini_write(config, config_file):
    ...
    for rule in config['smartdns']['rules']:
        ...
        cfg.set(section, 'proxy', rule['proxy'])
        cfg.set(section, 'refresh', rule['refresh'])
    ...
    cfg.set('proxy', 'port', config['smartdns']['proxy']['port'])
    for name, value in config['smartdns']['upstreams'].items():
        section = 'dns_' + name
        cfg.add_section(section)
        cfg.set(section, 'ip', value['ip'])
        cfg.set(section, 'port', value['port'])
        cfg.set(section, 'timeout', value['timeout'])
        cfg.set(section, 'proxy', value['proxy'])
    ...
    for domain in config['domains']:
        ...
        cfg.set(section, 'proxy', domain['proxy'])
        cfg.set(section, 'type', domain['type'])
        cfg.set(section, 'refresh', domain['refresh'])
